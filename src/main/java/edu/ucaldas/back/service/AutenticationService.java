 ...
 * Implements the {@link UserDetailsService} interface to provide custom user authentication logic.
 * 
 * This service is responsible for loading user details based on the provided login information.
 * It interacts with the {@link IUserRepository} to fetch user data from the database.
 * 
 * Dependencies:
 * - {@link IUserRepository}: Repository for accessing user data.
 * 
 * Methods:
 * - {@code loadUserByUsername(String login)}: Loads user details by email and ensures the user is active.
 * 
 * Annotations:

...
/**
 * SecurityFilter is a custom filter that extends OncePerRequestFilter to enforce security measures
 * on incoming HTTP requests. It validates the presence and correctness of an authentication token
 * in the "Authorization" header and sets the authentication in the security context if the token
 ...
 *   <li>Sets the authentication in the security context if the user is found and active.</li>
 *   <li>Throws appropriate exceptions for missing, invalid tokens, or inactive users.</li>
 * </ul>
 * 
 * <p><b>Dependencies:</b></p>
 * <ul>
 *   <li>{@code TokenService}: A service for token validation and extracting the subject (email).</li>
 *   <li>{@code IUserRepository}: A repository for retrieving user details by email.</li>
 * </ul>
 * 
 * <p><b>Exceptions:</b></p>
 * <ul>
 *   <li>{@code RuntimeException}: Thrown if the token is missing or invalid.</li>
 *   <li>{@code UsernameNotFoundException}: Thrown if the user is not found or inactive.</li>
 * </ul>
 * 
 * <p><b>Usage:</b></p>
 * <p>This filter is typically used in a Spring Security configuration to intercept and validate
 * requests before they reach the application endpoints.</p>
 * 
 * <p><b>Example:</b></p>
 * <pre>
 * {@code
...
public class SecurityFilter extends OncePerRequestFilter{
     ...
     * If the header is present and starts with "Bearer ", it extracts the token and validates it.
     * Upon successful validation, it retrieves the user details and sets the authentication
     * in the security context. If the token is missing, invalid, or the user is not found or inactive,
     ...
     * @throws RuntimeException if the token is missing, invalid, or the user is not found or inactive
     */
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) {
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            ...
            if (userEmail != null) {
                // Set the authentication in the security context or perform any other necessary actions
